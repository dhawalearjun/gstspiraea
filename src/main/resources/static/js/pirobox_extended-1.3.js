...
//								'<a href="#prev" class="piro_prev" title="previous"></a>'+
	...
	$('body').append(struct);
	$('body').append('<div class="piro_thumbs"></div>');
	...
	for (var i=0; i<my_gall_obj.length; i++) {
		...
		map['.'+ it.attr('class').match(/^pirobox\w*/)]=0;
	}
	...
	for (var i=0; i<gall_settings.length; i++) {
		//alert(gall_settings.length)
		if($(gall_settings[i]) == '.pirobox' || $(gall_settings[i]).length == 1){
			//alert(gall_settings[i])
			$(gall_settings[i]).addClass('single_fix');
			
		}else if ($(gall_settings[i]).length > 1){
			//alert(gall_settings[i] +'-'+gall_settings[i].length)
	...
	if(opt.zoom_animation == true){
		$('body').append('<script type="text/javascript" src="js/easing.js"></script>');
	}
	var piro_gallery = $(my_gall_obj);
	opt.padding = $('.piro_html .h_t_l').width();	
	$('.piro_html .h_mb_c,.nav_container').animate({height:0},0);
	wrapper.css({
		left:  (x/2)-(wrapper.width()/2),
		top: parseInt($(document).scrollTop())+(y-wrapper.height())/2
	});
...
//				for(var get_it in get_class){
//					if(get_class[get_it].indexOf('pirobox_gall') == 0){
//					var increase = 0;
//					$('.'+get_class[get_it]).each(function(){
//						increase += 100;
//						var this_src = $(this).find('img').attr('src');
//						var this_href = $(this).attr('href');
//						if(valid_extensions.test(this_href)){
//							//$('.piro_thumbs').html('');
//							$(this).clone(true).appendTo('.piro_thumbs');
//							
//							//alert(get_class[get_it]+'-'+increase);
//							//.addClass('pirobox_gall_over').removeAttr('rev').attr('rev',increase);
//						}
//					});
//				}
//			}
			//alert(single_class)
//			$(gall_settings).each(function(){});
//				var valid_extensions = /(\.jpg|\.jpeg|\.png|\.gif)$/i; 
//				$('<div class="piro_thumbnails"  />').appendTo('.nav_big');
//				var imgs_t = $(this).attr('href');
//				var imgs_class = $(this).prop('class');
//				if(valid_extensions.test(imgs_t)){
//					$(
//					'<a href="'+imgs_t+'" class="pirobox_gall_T" data-pirobox="gallery">'+
//					'<img src="'+imgs_t+'" />'+
//					'</a>'
//					).appendTo('.piro_thumbnails');
//				}
			
			//alert($(piro_gallery).attr('class'));
			piro_bg.css({'opacity':opt.bg_alpha});	
			e.preventDefault();
		...
		function open_all(){
			...
			function animate_html(){
				...
				if( params[1] <= 1 && params[2] <= 1 ){ 
					params[2] = Math.ceil( $(window).height() * (params[2] )); 
					params[1] = Math.ceil( $(window).width() * (params[1] )); 
				} 
				var y = $(window).height();
				var x = $(window).width();
			...
			function animate_image (){
				...
				piro_nav_in.hide();
				$('.nav_big').hide();
				$('.nav_container, .h_mb_c').css('height',0)
				if(descr == "" || descr === undefined || descr === false ){
					$('.piro_caption').hide();
				}else{
					$(piro_capt_cont).appendTo(piro_nav_in);
				...
				img.onload = function() {
					...
					resize.add(div_reg).animate({'height': imgH,'width': imgW},opt.piro_speed);
					wrapper.animate({
						'height' : imgH+(opt.padding*2),
						'width' : imgW+(opt.padding*2), 
						'top' : top,
						'left': ((x/2)-((imgW/2)+opt.padding))},opt.piro_speed,function(){
							...
							div_reg.append(img);
							$(img).fadeIn(700,function(){
								$('.nav_container, .h_mb_c').animate({height:41},200);
								piro_nav_in.show();
								$(window).scroll(function(){
									get_position();
								...
								zoomIn.bind('click',function(w){
									...
									$(img).animate({
										...
										'height': this_h , 
										'top':-(this_h-imgH)/2, 
										'left':-(this_w-imgW)/2 },600,function(){
											if(opt.move_mode == 'drag'){
													...
													var	div_reg_w = div_reg.width(),
														...
														div_reg.bind('mousemove',function(e){
															...
															if (Math.abs(last_x-pos_x)<=1 && Math.abs(last_y-pos_y)<=1) return;
																...
																last_y = pos_y;
															if (opt.zoom_animation == true){
																$(img).stop().animate({ left : -(perc_x*pos_x),top:-(perc_y*pos_y)},500,'easeOutCirc');
															}else{
...
//					var top = parseInt($(document).scrollTop())+(y-imgH)/2 + position;
//					$(img).height(imgH).width(imgW);
//					resize.add(div_reg).css({'height': imgH,'width': imgW});
//					wrapper.css({
//						'height' : imgH+(opt.padding*2),
//						'width' : imgW+(opt.padding*2), 
//						'top' : top,
